...
// Requires the Vite dev server on 127.0.0.1:5173 (npm run dev). Renders only the stair and the R15 under-stair composition.
const page=await browser.newPage({viewport:{width:1400,height:1000}});
await page.goto(process.env.DEV_SERVER??'http://127.0.0.1:5173/');
const views=[
 {name:'qa/R15-under-stair-detail.png',from:[3.5,2.55,3.1],to:[-1.75,1.65,.6]},       // from the passage: the fitted composition and the basin archway
 {name:'qa/R15-under-stair-wash.png',from:[.2,1.6,-.94],to:[-2.135,1.0,-.94]},          // straight down the run through the archway: R15 puts the basin square in front of you
 {name:'qa/R15-under-stair-panel.png',from:[1.5,3.1,2.7],to:[-1.3,1.25,.5]},            // from over the living side: the raking top edge of the backing panel, held 60 mm under the waist soffit
 {name:'qa/R15-under-stair-living-eye.png',from:[2.2,2.0,2.05],to:[-.9,1.5,.55],full:true},// eye level in the living passage, under the retained +2.10 header: the intended read
];
for(const v of views){
 await page.evaluate(async v=>{
  ...
  const names=['stairs','R14 fitted TV joinery under the lower flight','R15 washbasin in the lower-flight bay, continuing the TV run; basin faces east'];
  if(v.full){
...
await browser.close();console.log('Saved R15 under-stair detail views.');
